...
function GetParentId($cid=0)
{
	global $dosql;

	$r = $dosql->GetOne("SELECT `parentid` FROM `#@__infoclass` WHERE `id`=$cid");

	if((isset($r['parentid']))&&($r['parentid']!=0)){
		return $r['parentid'];
	}else if((isset($r['parentid']))&&($r['parentid']==0)){
		return $cid;
	}
	return 0;
}
...
function GetFirstChild($cid)
{
	...
	$str = '';
	$sql = "SELECT * FROM `#@__infoclass` WHERE parentid=$cid AND checkinfo=true ORDER BY orderid ASC"; 
