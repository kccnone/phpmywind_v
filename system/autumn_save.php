...
if($action == 'add')
{
	...
	$ids = GetDiyFieldCatePriv('8',$classid);
	if(!empty($ids))
	{
		$dosql->Execute("SELECT * FROM `#@__diyfield` WHERE infotype=8 AND `id` IN ($ids) AND checkinfo=true ORDER BY orderid ASC");
...
{
	//审核权限
	$r = $dosql->GetOne("SELECT `classid` FROM `#@__autumn` WHERE `id`=$id");
