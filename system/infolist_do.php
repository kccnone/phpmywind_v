	...
	while($row = $dosql->GetArray())
	{
		...
		if($cfg_adminlevel != 1)
		{
			if(in_array($row['classid'], $catgoryUpdatePriv)){
				$updateStr = '<a href="infolist_update.php?cid='.$cid.'&id='.$row['id'].'&flag='.$flag.'&page='.$page.'&keyword='.$keyword.'&ver='.$ver.'">修改</a>';
				$copyStr = '<a href="infolist_copy.php?cid='.$cid.'&id='.$row['id'].'">复制</a>';
			}
			else{
				$updateStr = '修改';
				$copyStr = '复制';
