...
if($action == 'add')
{
	...
	$ids = GetDiyFieldCatePriv('6',$classid);
	if(!empty($ids))
	{
		$dosql->Execute("SELECT * FROM `#@__diyfield` WHERE infotype=6 AND `id` IN ($ids) AND checkinfo=true ORDER BY orderid ASC");
...
{
	//审核权限
	$r = $dosql->GetOne("SELECT `classid` FROM `#@__spring` WHERE `id`=$id");
