...
<form name="form" id="form" method="post" action="message_save.php">
	<div class="toolbarTab" >
    	<ul>
        	<li class="<?php if(empty($ischeck)) echo "on"; ?>" ><a href="message.php" >全部</a></li>
            <li class="line" > - </li>
            <li class="<?php if((!empty($ischeck))&&($ischeck=='false')) echo "on"; ?>"><a href="message.php?ischeck=false" >未审</a></li>
            <li class="line" > - </li>
            <li class="<?php if((!empty($ischeck))&&($ischeck=='true')) echo "on"; ?>"><a href="message.php?ischeck=true" >已审</a></li>
        </ul>
    </div>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		...
		$messql="";
		if(!empty($ischeck)){
			$messql = "SELECT * FROM `#@__message` WHERE `siteid`='$cfg_siteid' AND checkinfo='$ischeck'";	
		}
		else{
			$messql = "SELECT * FROM `#@__message` WHERE `siteid`='$cfg_siteid'";
		}
		$dopage->GetPage($messql);
