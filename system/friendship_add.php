...
<body>
<div class="formHeader"> <span class="title">添加信息</span> <a href="javascript:location.reload();" class="reload">刷新</a> </div>
<form name="form" id="form" method="post" action="friendship_save.php" onsubmit="return cfm_infolm();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
		...
		<tr>
			...
			$row = $dosql->GetArray();
			if(is_array($row)){
				echo '<span><input type="radio" name="ver" id="ver" value="'.$row['flag'].'" checked="checked" />'.$row['flagname'].'</span>';	
			}
			while($row = $dosql->GetArray())
			{
				echo '<span><input type="radio" name="ver" id="ver" value="'.$row['flag'].'" />'.$row['flagname'].'</span>';
			}
			?></td>
		</tr>
		<tr class="nb">
		...
		<tr class="nb">
			<td colspan="2" height="0" id="df"><?php if(isset($cid)) echo GetDiyField(14,$cid); ?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input type="text" name="orderid" id="orderid" class="inputos" value="<?php echo GetOrderID('#@__friendship'); ?>" /></td>
		</tr>
		<tr>
