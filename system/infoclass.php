...
<form name="form" id="form" method="post" action="">
	...
	function Show($id=0, $i=0)
	{
		global $dosql,$cfg_siteid,$cfg_adminlevel,
			   ...
			   $catgoryUpdatePriv,$catgoryDelPriv,$catgoryYouhPriv;
		...
		while($row = $dosql->GetArray($id))
		{
			...
			if($cfg_adminlevel != 1)
			{
				if(in_array($row['id'], $catgoryUpdatePriv))
					$updateStr = '<a href="infoclass_update.php?infotype='.$row['infotype'].'&id='.$row['id'].'">修改</a>';
				else
					$updateStr = '修改';
			}
			else
			...
			if($cfg_adminlevel != 1)
			{
				if(in_array($row['id'], $catgoryYouhPriv))
					$youhStr = '<a href="infoclass_youh.php?infotype='.$row['infotype'].'&id='.$row['id'].'">优化</a>';
				else
					$youhStr = '优化';
			}
			else
	...
	<div rel="rowpid_<?php echo GetTopID($row['parentstr']); ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
			<tr align="left" class="dataTr">
				...
				<td width="32%" class="action endCol"><span><?php echo $addStr; ?></span> | <span><?php echo $checkStr; ?></span> | <span><?php echo $updateStr; ?></span> | <span><?php echo $youhStr; ?></span> | <span class="nb"><?php echo $delStr; ?></span></td>
			</tr>
		</table>
	</div>
	<?php
