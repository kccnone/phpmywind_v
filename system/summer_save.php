...
if($action == 'add')
{
	...
	$ids = GetDiyFieldCatePriv('7',$classid);
	if(!empty($ids))
	{
		$dosql->Execute("SELECT * FROM `#@__diyfield` WHERE infotype=7 AND `id` IN ($ids) AND checkinfo=true ORDER BY orderid ASC");
...
{
	//审核权限
	$r = $dosql->GetOne("SELECT `classid` FROM `#@__summer` WHERE `id`=$id");
