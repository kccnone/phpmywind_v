...
<body>
<?php
$row = $dosql->GetOne("SELECT * FROM `#@__friendship` WHERE `id`=$id");
?>
<div class="formHeader"> <span class="title">修改信息</span> <a href="javascript:location.reload();" class="reload">刷新</a> </div>
<form name="form" id="form" method="post" action="friendship_save.php" onsubmit="return cfm_infolm();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="formTable">
		...
		<tr>
			...
			<td><select name="mainid" id="mainid">
					...
					<?php GetAllType('#@__maintype','#@__friendship','mainid'); ?>
				</select>
		...
		<tr class="nb">
			...
			echo GetDiyField('14',$row['classid'],$row);
			?></td>
		</tr>
		<tr>
		...
		<tr class="nb">
			...
			<td><fieldset class="picarr">
					...
					if($row['picarr'] != '')
					{
						...
						{
							$v = explode(',', $v);
							echo '<li rel="'.$v[0].'"><div><img style="height:50px;" src="../'.$v[0].'"></div><input type="text" name="picarr[]" value="'.$v[0].'"><a href="javascript:void(0);" onclick="ClearPicArr(\''.$v[0].'\')">删除</a><br /><input type="text" name="picarr_txt[]" value="'.$v[1].'"><span>描述</span></li>';
						}
					}
					?>
					</ul>
	...
	<div class="formSubBtn">
        ...
        <input type="hidden" name="verc" id="verc" value="<?php echo $ver; ?>" />       
	</div>
</form>
<script>
    $(function()
