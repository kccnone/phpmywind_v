...
if($action == 'add')
{
	...
	$ids = GetDiyFieldCatePriv('9',$classid);
	if(!empty($ids))
	{
		$dosql->Execute("SELECT * FROM `#@__diyfield` WHERE infotype=9 AND `id` IN ($ids) AND checkinfo=true ORDER BY orderid ASC");
...
{
	//审核权限
	$r = $dosql->GetOne("SELECT `classid` FROM `#@__winter` WHERE `id`=$id");
