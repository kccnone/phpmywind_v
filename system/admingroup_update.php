		...
		<tr>
			...
			?>
				...
				<div class="purviewList"> <span>
					...
					级联数据管理</span> </div>
				<div class="purviewTitle"><strong>产品订单管理</strong></div>
				<div class="purviewList"> <span>
...
function Show($siteid=1, $groupid=0, $id=0, $i=0)
{
	...
	while($row = $dosql->GetArray($id))
	{
		...
		$r_listc = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=$groupid AND `siteid`=$siteid AND `model`='category' AND `classid`=".$row['id']." AND `action`='listc'");
		if(isset($r_listc) && is_array($r_listc))
		{
			$cklistc = 'checked="checked"';
			$cktop = 'checked="checked"';
		}
		else
		{
		...
		$r_addc = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=$groupid AND `siteid`=$siteid AND `model`='category' AND `classid`=".$row['id']." AND `action`='addc'");
		if(isset($r_addc) && is_array($r_addc))
		{
			$ckaddc = 'checked="checked"';
			$cktop = 'checked="checked"';
		}
		else
		{
		...
		$r_updatec = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=$groupid AND `siteid`=$siteid AND `model`='category' AND `classid`=".$row['id']." AND `action`='updatec'");
		if(isset($r_updatec) && is_array($r_updatec))
		{
			$ckupdatec = 'checked="checked"';
			$cktop = 'checked="checked"';
		}
		else
		{
		...
		$r_delc = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=$groupid AND `siteid`=$siteid AND `model`='category' AND `classid`=".$row['id']." AND `action`='delc'");
		if(isset($r_delc) && is_array($r_delc))
		{
			$ckdelc = 'checked="checked"';
			$cktop = 'checked="checked"';
		}
		else
		{
		...
		$r_youhc = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=$groupid AND `siteid`=$siteid AND `model`='category' AND `classid`=".$row['id']." AND `action`='youhc'");
		if(isset($r_youhc) && is_array($r_youhc))
		{
			$ckyouhc = 'checked="checked"';
			$cktop = 'checked="checked"';
		}
		else
		{
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
		<tr align="left" class="dataTr">
			...
			<td width="55%" class="privTxt">
				...
				删除</span>
                <span>
				<input type="checkbox" name="priv[<?php echo $siteid; ?>][<?php echo $row['id']; ?>][]" value="listc" <?php echo $cklistc; ?> />
				查看栏目</span> <span>
				<input type="checkbox" name="priv[<?php echo $siteid; ?>][<?php echo $row['id']; ?>][]" value="addc" <?php echo $ckaddc; ?> />
				添加栏目</span> <span>
				<input type="checkbox" name="priv[<?php echo $siteid; ?>][<?php echo $row['id']; ?>][]" value="updatec" <?php echo $ckupdatec; ?> />
				修改栏目</span> <span>
				<input type="checkbox" name="priv[<?php echo $siteid; ?>][<?php echo $row['id']; ?>][]" value="delc" <?php echo $ckdelc; ?> />
				删除栏目</span>
                <span>
				<input type="checkbox" name="priv[<?php echo $siteid; ?>][<?php echo $row['id']; ?>][]" value="youhc" <?php echo $ckyouhc; ?> />
				优化栏目</span>
           	</td>
		</tr>
	</table>
