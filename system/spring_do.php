<?php	require_once(dirname(__FILE__).'/inc/config.inc.php');IsModelPriv('spring');
	...
	while($row = $dosql->GetArray())
	{
		...
		if($cfg_adminlevel != 1)
		{
			if(in_array($row['classid'], $catgoryUpdatePriv)){
				$updateStr = '<a href="spring_update.php?cid='.$cid.'&id='.$row['id'].'&flag='.$flag.'&page='.$page.'&keyword='.$keyword.'&ver='.$ver.'">修改</a>';
				$copyStr = '<a href="spring_copy.php?cid='.$cid.'&id='.$row['id'].'">复制</a>';
			}
			else{
				$updateStr = '修改';
				$copyStr = '复制';
...
?>
<div class="bottomToolbar"> <span class="selArea"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:;" onclick="AjaxClearAll();">删除</a></span> <a href="spring_add.php" class="dataBtn">添加列表信息</a></span> </div>
...
<div class="quickToolbar">
	<div class="qiuckWarp">
		<div class="quickArea"><span class="selArea"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:;" onclick="AjaxClearAll();">删除</a></span> <a href="spring_add.php" class="dataBtn">添加列表信息</a> <span class="pageSmall"><?php echo $dopage->AjaxPageSmall(); ?></span> </div>
